...
import { generateToken, generateRefreshToken, getRefreshTokenExpiry } from '../middleware/auth'
import { registerSchema, loginSchema, refreshTokenSchema } from '../schemas/auth'
import { ValidationError, AuthError, ConflictError, RateLimitError } from '../utils/error-mapper'
...
auth.post('/register', registerRateLimiter, async (c) => {
  try {
    ...
    const existing = await db.prepare('SELECT id FROM users WHERE username = ?').bind(validated.username).first()
    if (existing) {
    ...
    const accessToken = generateToken(userId, validated.username, c.env.JWT_SECRET)
...
auth.post('/login', loginRateLimiter, async (c) => {
  try {
    ...
    const user = await db
      .prepare('SELECT id, username, password_hash, locked_until, failed_login_attempts FROM users WHERE username = ?')
      .bind(validated.username)
      .first() as { id: number; username: string; password_hash: string; locked_until: string | null; failed_login_attempts: number } | undefined
    ...
    if (user.locked_until) {
      ...
      if (lockedUntil > new Date()) {
        ...
        throw new RateLimitError(`Account temporarily locked. Try again in ${waitSeconds} seconds.`)
      }
      // Lock period expired, reset
      await db
        .prepare('UPDATE users SET locked_until = NULL, failed_login_attempts = 0 WHERE id = ?')
        .bind(user.id)
        .run()
    ...
    if (!passwordMatch) {
      ...
      if (lockedUntilValue) {
        throw new RateLimitError('Too many failed login attempts. Account locked for 30 minutes.')
      }
...
auth.post('/refresh', async (c) => {
  try {
    ...
    // Generate new tokens
    const newAccessToken = generateToken(storedToken.user_id, user.username, c.env.JWT_SECRET)
