...
function isLineComplete(grid: number[][], lineIndex: number, lineType: 'row' | 'col'): boolean {
  if (lineType === 'row') {
...
grid.get('/today', verifyAuth, async (c) => {
  try {
    ...
    let dailyGrid = await db
      ...
      .first() as { id: number; user_id: number; grid_date: string; grid_data: string; created_at: string } | undefined

    if (!dailyGrid) {
      // Generate new grid
      const items = await db.prepare('SELECT id, content FROM bingo_items ORDER BY RANDOM() LIMIT 16').all() as { results: Array<{ id: number; content: string }> }
      ...
      const gridData = {
        items: selectedItems.map(item => ({ id: item.id, content: item.content, marked: false })),
        createdAt: new Date().toISOString()
      }

      const result = await db
        .prepare('INSERT INTO daily_grids (user_id, grid_date, grid_data, created_at) VALUES (?, ?, ?, ?)')
        .bind(user.userId, today, JSON.stringify(gridData), new Date().toISOString())
        .run()
...
grid.patch('/mark', verifyAuth, async (c) => {
  try {
    ...
    const gridArray = []
    for (let i = 0; i < 4; i++) {
      gridArray.push(gridData.items.slice(i * 4, (i + 1) * 4).map((it: BingoItem) => it.marked ? 1 : 0))
    }
    ...
    if (marked && !oldMarked) {
      ...
      for (let i = 0; i < 4; i++) {
        if (isLineComplete(gridArray, i, 'row')) {
          ...
          const existing = await db
            .prepare('SELECT id FROM completed_rows WHERE user_id = ? AND grid_date = ? AND row_type = ? AND row_index = ?')
          ...
          if (!existing) {
            ...
            bingoCountToAdd += 1
            await db
              .prepare('INSERT INTO completed_rows (user_id, grid_date, row_type, row_index, created_at) VALUES (?, ?, ?, ?, ?)')
      ...
      if (isFullCardComplete(gridArray)) {
        const existing = await db
          .prepare('SELECT id FROM completed_rows WHERE user_id = ? AND grid_date = ? AND row_type = ?')
      ...
      if (pointsToAdd > 0) {
        await db
          .prepare('UPDATE user_scores SET points = points + ?, bingo_count = bingo_count + ?, updated_at = ? WHERE user_id = ?')
          .bind(pointsToAdd, bingoCountToAdd, new Date().toISOString(), user.userId)
          .run()
      }
    }
